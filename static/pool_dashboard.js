...
(() => {
  ...
  const FEED_MODE_REJECTS = "rejects";
  const FEED_MODE_BLOCKS = "blocks";
  const FEED_MODE_PAYOUTS = "payouts";
  const FEED_MODE_BANNED = "banned";
  ...
  const state = {
    ...
    // optional backend support (future): track banned miners (map by minerId)
    bannedMiners: {},
  ...
  const feed = el("feed");
  const feedViewport = el("feed-viewport");

  const htmCard = el("howtomine-card");
  const htmCollapseBtn = el("htm-collapse-btn");
  ...
  const feedCard = el("feed-card");
  const feedCollapseBtn = el("feed-collapse-btn");
  ...
  const feedRejects = el("feed-rejects");
  const feedBlocks = el("feed-blocks");
  const feedPayouts = el("feed-payouts");
  const feedBanned = el("feed-banned");
  const feedMinerOnly = el("feed-miner-only");
  const feedRows5 = el("feed-rows-5");
  const feedRows10 = el("feed-rows-10");
  const feedRows20 = el("feed-rows-20");
  ...
  function applySnapshot(snap){
    ...
    if(Array.isArray(bansList)){
      ...
      for(const b of bansList){
        ...
        out[miner]={
          miner,
          banned_at: pick(b,["banned_at","bannedAt","ts","timestamp"]),
          unbanned_at: isUnban ? (pick(b,["unbanned_at","unbannedAt"])||b.timestamp||null) : null,
          until: pick(b,["until","expires_at","expiresAt"]),
          reason: String(pick(b,["reason","msg","message"])??"").trim(),
          active: !isUnban,
        };
      }
      state.bannedMiners=out;
    }
    ...
    if(!state.initialSnapshotApplied){
      ...
      const reSharesAcc=snap.recent_shares_acc||snap.recentSharesAcc||null;
      const reSharesRej=snap.recent_shares_rej||snap.recentSharesRej||null;
      const reBlocks=snap.recent_blocks||snap.recentBlocks||null;
      const rePayouts=snap.recent_payouts||snap.recentPayouts||null;
      const reBans=snap.recent_bans||snap.recentBans||null;
      if(Array.isArray(reSharesAcc)&&reSharesAcc.length) for(const e of reSharesAcc) merged.push(e);
      if(Array.isArray(reSharesRej)&&reSharesRej.length) for(const e of reSharesRej) merged.push(e);
      ...
      if(Array.isArray(reBans)&&reBans.length) for(const e of reBans) merged.push(e);
  ...
  function loadUiPrefs(){
    ...
    if(obj&&typeof obj==="object"){
      ...
      if(typeof obj.htmCollapsed==="boolean") state.htmCollapsed=obj.htmCollapsed;
      if(typeof obj.feedCollapsed==="boolean") state.feedCollapsed=obj.feedCollapsed;
      if(obj.theme==="light"||obj.theme==="dark") state.theme=obj.theme;
      if([5,10,20].includes(Number(obj.feedRows))) state.feedRows=Number(obj.feedRows);
    }
  }
  function saveUiPrefs(){ try{ localStorage.setItem(UI_KEY,JSON.stringify({minerCollapsed:state.minerCollapsed,htmCollapsed:state.htmCollapsed,feedCollapsed:state.feedCollapsed,theme:state.theme,feedRows:state.feedRows})); } catch {} }
  function applyTheme(){
    document.body.setAttribute("data-theme",state.theme);
    if(themeToggle) themeToggle.textContent = state.theme==="light" ? "Theme: Light" : "Theme: Dark";
  }
  ...
  function feedModeMatches(e){
    const mode=state.feedMode;
    if(mode===FEED_MODE_ALL) return true;
    if(mode===FEED_MODE_SHARES) return e.cat===FEED_MODE_SHARES;
    if(mode===FEED_MODE_REJECTS) return e.typeClass==="reject";
    if(mode===FEED_MODE_BLOCKS) return e.cat===FEED_MODE_BLOCKS;
    if(mode===FEED_MODE_PAYOUTS) return e.cat===FEED_MODE_PAYOUTS;
    if(mode===FEED_MODE_BANNED) return e.cat===FEED_MODE_BANNED;
    return true;
  }
  ...
  function updateFeedChips(){
    ...
    feedRejects.classList.toggle("on",state.feedMode===FEED_MODE_REJECTS);
    ...
    feedBanned.classList.toggle("on",state.feedMode===FEED_MODE_BANNED);
    feedMinerOnly.classList.toggle("on",!!state.feedMinerOnly);
    feedRows5.classList.toggle("on",state.feedRows===5);
    feedRows10.classList.toggle("on",state.feedRows===10);
    feedRows20.classList.toggle("on",state.feedRows===20);
  }

  function applyFeedRows(){
    const rows=[5,10,20].includes(state.feedRows)?state.feedRows:10;
    if(feedViewport) feedViewport.style.setProperty("--feed-rows",String(rows));
  }
  ...
  function drawBarChart(ctx,canvas,labels,values,opts){
    ...
    const padL=10,padR=10,padT=22,padB=22,innerW=w-padL-padR,innerH=h-padT-padB;
    ...
    for(let i=0;i<n;i++){
      ...
      /* ── v1.7.0: block count label ABOVE bar ── */
      if(v>0){
        ctx.fillStyle="rgba(255,255,255,0.92)";
        ctx.font="bold 11px ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, 'Liberation Mono', 'Courier New', monospace";
        const lbl=String(v);
        const tw=ctx.measureText(lbl).width;
        ctx.fillText(lbl, x+(barW-tw)/2, y-4);
      }
  ...
  function drawHashrateChart(){
    ...
    const dense=poolPts.length>500||netPts.length>500;

    // ── Draw network (behind pool) ──
    if(netPts.length>=2){
      // Dashed line
      hrChartCtx.strokeStyle=dense?"rgba(111,231,255,0.70)":"rgba(111,231,255,0.50)"; hrChartCtx.lineWidth=1.5;
    ...
    // ── Draw pool (on top) ──
    if(poolPts.length>=2){
      // Thinner line + lighter fill when many points (24h)
      // Line
      hrChartCtx.strokeStyle=dense?"rgba(255,184,107,0.50)":"rgba(255,184,107,0.75)";
      hrChartCtx.lineWidth=dense?1:2;
      hrChartCtx.beginPath();
      ...
      hrChartCtx.closePath();
      hrChartCtx.fillStyle=dense?"rgba(255,184,107,0.04)":"rgba(255,184,107,0.10)";
      hrChartCtx.fill();
    }
    ...
    // Left: pool scale
    if(poolPts.length>=2){
      hrChartCtx.fillStyle="rgba(255,184,107,0.7)";
  ...
  function setMinerView(minerId){
    ...
    else { mMiner.textContent=id; mConnected.textContent=m.connectedSeen?"yes":"no"; const pAcc=(state.minerSharesAcc&&state.minerSharesAcc[id])?Number(state.minerSharesAcc[id]):0; const pRej=(state.minerSharesRej&&state.minerSharesRej[id])?Number(state.minerSharesRej[id]):0; const dAcc=Math.max(m.acc,pAcc); const dRej=Math.max(m.rej,pRej); mShares.textContent=`${dAcc} / ${dRej}`; mLastH.textContent=(m.lastHeight!==null&&m.lastHeight!==undefined)?String(m.lastHeight):"—"; mHR.textContent=fmtHashrate(computeHashrateFromTs(m.shareTimesMs)); }
    if(!m){ const pAcc2=(state.minerSharesAcc&&state.minerSharesAcc[id])?Number(state.minerSharesAcc[id]):0; const pRej2=(state.minerSharesRej&&state.minerSharesRej[id])?Number(state.minerSharesRej[id]):0; if(pAcc2||pRej2) mShares.textContent=`${pAcc2} / ${pRej2}`; }
  ...
  function applyHtmCollapsed(){
    htmCard.classList.toggle("collapsed",!!state.htmCollapsed);
    htmCollapseBtn.textContent=state.htmCollapsed?"Expand":"Collapse";
  ...
  function applyFeedCollapsed(){
    feedCard.classList.toggle("collapsed",!!state.feedCollapsed);
    feedCollapseBtn.textContent=state.feedCollapsed?"Expand":"Collapse";
    setTimeout(()=>markChartsDirty("all"),0);
  }
  ...
  function ingestEvent(ev){
    ...
    switch(ev.type){
      ...
      // ── Ban events (supports several common names) ──
      case "MinerBanned":
      case "Ban":
      case "Banned": {
  ...
  htmCollapseBtn.addEventListener("click",()=>{ state.htmCollapsed=!state.htmCollapsed; saveUiPrefs(); applyHtmCollapsed(); });
  minerCollapseBtn.addEventListener("click",()=>{ state.minerCollapsed=!state.minerCollapsed; saveUiPrefs(); applyMinerCollapsed(); });
  feedCollapseBtn.addEventListener("click",()=>{ state.feedCollapsed=!state.feedCollapsed; saveUiPrefs(); applyFeedCollapsed(); });
  ...
  feedRejects.addEventListener("click",()=>{ state.feedMode=FEED_MODE_REJECTS; updateKpis(); renderFeed(); });
  ...
  feedBanned.addEventListener("click",()=>{ state.feedMode=FEED_MODE_BANNED; updateKpis(); renderFeed(); });
  feedMinerOnly.addEventListener("click",()=>{ state.feedMinerOnly=!state.feedMinerOnly; updateKpis(); renderFeed(); });
  feedRows5.addEventListener("click",()=>{ state.feedRows=5; applyFeedRows(); saveUiPrefs(); updateKpis(); });
  feedRows10.addEventListener("click",()=>{ state.feedRows=10; applyFeedRows(); saveUiPrefs(); updateKpis(); });
  feedRows20.addEventListener("click",()=>{ state.feedRows=20; applyFeedRows(); saveUiPrefs(); updateKpis(); });
  ...
  // Boot
  // -------------------------------------------------------------------------
  // Fetch pool connect address from static file
  (async function loadPoolAddress(){
    try{
      const res=await fetch("/static/pool-address",{cache:"no-store"});
      if(res&&res.ok){
        const addr=(await res.text()).trim();
        if(addr){
          document.getElementById("pool-connect-addr").textContent=addr;
          const toml=document.getElementById("htm-toml-code");
          if(toml) toml.textContent=toml.textContent.replace('address = ""','address = "'+addr+'"');
          document.querySelectorAll(".pool-address-display").forEach(el=>el.textContent=addr);
        }
      }
    } catch {}
  })();
